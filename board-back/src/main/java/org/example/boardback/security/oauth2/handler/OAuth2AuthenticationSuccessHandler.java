...
public class OAuth2AuthenticationSuccessHandler extends SimpleUrlAuthenticationSuccessHandler {
    ...
    // 리프레시 토큰을 담을 쿠키 이름 상수: HttpONLY 쿠키(프론트) - 개발자가 건드리지 못함
    private static final String REFRESH_TOKEN = "refreshToken";

    @Override
    public void onAuthenticationSuccess( // SimpleUrlAuthenticationSuccessHandler 안에 있음
        ...
        // SecurityContext에 저장된 principal(UserDetails 구현체)을 꺼냄 - JWT 토큰생성을 위한 User 꺼내기 코드
        UserPrincipal principal = (UserPrincipal) authentication.getPrincipal(); // Object -> UserPrincipal로 형변환
        ...
        // principal이 가진 권한(ROLE들)을 문자열 Set 으로 추출
        Set<String> roles = principal.getAuthorities().stream()
                .map(GrantedAuthority::getAuthority)            // ROLE_USER 같은 문자열로 변환
                .collect(Collectors.toSet());                   // 중복이 없는 Set으로 수정
